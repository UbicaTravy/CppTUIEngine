...
Движок имеет помощника для сборки и запуска проекта. Но сначало его нужно собрать. Зайдите в папку `helper` и выполните файл `comp.cmd`. .ехе файл не идёт с этим помощником из-за различия архитектуры и для удобства менять команды в нём или добавлять что-то своё.

После сборки в главной папке проекта появиться `build_tool.exe`, запустите его. Выберете стрелочками пункт `Build and Run`. Если нету ошибок и установлен `g++` то проект соберётся и запуститься.
